...
if($c != ""){ 
$c = testQty($c);
$act = $db->delete("admin_messages", " id='{$c}' AND recipient_email = '$user_email' AND inbox = '1'");	
$i++;			
}else{
continue;
}
...
$result = $db->select("admin_messages", "WHERE recipient_email = '$user_email' AND inbox = '1'", "*", "ORDER BY id DESC");

$per_view = 20;
$page_link = "{$admin}inbox?pn=";
$link_suffix = "";
...
$result = $db->select("admin_messages", "WHERE recipient_email = '$user_email' AND inbox = '1'", "*", "ORDER BY id DESC", "LIMIT {$offset},{$per_view}");

if(count_rows($result) > 0){
?>
<form action="<?php echo $admin; ?>inbox" class="general-form" id="form-div" method="post" runat="server" autocomplete="off" enctype="multipart/form-data" style="overflow-x:auto;">
...
<td><?php echo $date; ?></td>
<td><a href="<?php echo $admin; ?>inbox?view=<?php echo $msg_id; ?>&pn=<?php echo $pn; ?>" class="btn gen-btn general-link" title="View message #<?php echo $msg_id; ?>"><i class="fa fa-eye" aria-hidden="true"></i> View</a></td>
...
$result = $db->select("admin_messages", "WHERE id='$view' AND recipient_email = '$user_email' AND inbox = '1'", "*", "");
...
<div class="back"><a href="<?php echo $admin; ?>inbox?pn=<?php echo $pn; ?>" class="btn gen-btn general-link"><i class="fa fa-arrow-left"></i> Back to inbox</a></div>
