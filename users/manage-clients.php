...
<?php
admin_role_redirect("manage_clients");
...
////////////// Upload image //////////////////////////////
if(check_admin("edit_clients") == 1 && $_SERVER['REQUEST_METHOD'] == "POST" && !empty($upload) && !empty($member_id) && !empty($_FILES["ufile"]["tmp_name"])){ 
...
if(check_admin("add_clients") == 1 && $_SERVER['REQUEST_METHOD'] == "POST" && !empty($add) && !empty($name) && !empty($new_username) && !empty($password) && !empty($re_password) && strlen($password) >= 5 && $password == $re_password && !empty($email) && !empty($re_email) && filter_var($email, FILTER_VALIDATE_EMAIL) && $email == $re_email && $check_user == $_SESSION["spam_checker"]){
...
if(check_admin("edit_clients") == 1 && $_SERVER['REQUEST_METHOD'] == "POST" && !empty($edit) && !empty($name) && !empty($email)){
...
$act = $db->update($data_array, "reg_users", "id = '$edit' AND client = '1'");

if($act){

$activity = "Updated the profile of user #" . client_id($edit) . ".";
$audit_data_array = array(
"user_id" => "'$id'",
"name" => "'$username'",
"email" => "'$user_email'",
...
if(check_admin("edit_clients") == 1 && $_SERVER['REQUEST_METHOD'] == "GET" && (!empty($unblock) || !empty($block))){
...
if($act){
$activity = (!empty($unblock))?"Unblocked user #" . client_id($uniq_id) . " to gain login access.":"Blocked user #" . client_id($uniq_id) . " from logging in.";
$audit_data_array = array(
"user_id" => "'$id'",
"name" => "'$username'",
"email" => "'$user_email'",
...
//////////////////////////////////=====Activate User============//////////////////////////////////////////
if(check_admin("edit_clients") == 1 && $_SERVER['REQUEST_METHOD'] == "GET" && !empty($activate)){
...
if($act){
$activity = "Activated a user - {$uniq_name}({$uniq_email}) with ID #" . client_id($uniq_id) . ".";
$audit_data_array = array(
"user_id" => "'$id'",
"name" => "'$username'",
"email" => "'$user_email'",
...
if(check_admin("edit_clients") == 1 && $_SERVER['REQUEST_METHOD'] == "POST" && !empty($reset) && !empty($password) && !empty($conf_password) && $password == $conf_password && strlen($password) >= 5){
...
$activity = "Reset password for user #" . client_id($uniq_id) . ".";
$audit_data_array = array(
"user_id" => "'$id'",
"name" => "'$username'",
"email" => "'$user_email'",
...
$page_link = "{$admin}manage-clients?pn=";
...
<div class="page-title">Manage Clients 

<?php if(check_admin("add_clients") == 1){ ?>
<a href="<?php echo $admin; ?>manage-clients?add=1&pn=<?php echo $pn; ?>" class="btn gen-btn general-link float-right"><i class="fa fa-upload" aria-hidden="true"></i> New Client</a>
<?php } ?>
</div>

<form action="<?php echo $admin; ?>manage-clients" class="img-form general-form" id="form-div" method="post" runat="server" autocomplete="off" enctype="multipart/form-data">  
...
<form action="<?php echo $admin; ?>manage-clients" class="general-form" id="form-div" method="post" runat="server" autocomplete="off" enctype="multipart/form-data">
<input type="hidden" name="gh" value="1"> 
...
<th>Activities</th>
<th>Profile</th>
<?php if(check_admin("change_clients_picture") == 1){ ?>
<th>Image</th>
<?php } ?>
</tr>
</thead>
...
<td><a href="<?php echo $admin; ?>manage-clients?activities=<?php echo $get_id; ?>&pn=<?php echo $pn; ?>" class="btn gen-btn general-link" title="View <?php echo $name; ?>&#039;s activities"><i class="fa fa-history" aria-hidden="true"></i> View Log</a></td>
<td><a href="<?php echo $admin; ?>manage-clients?view=<?php echo $get_id; ?>&pn=<?php echo $pn; ?>" class="btn gen-btn general-link" title="View <?php echo $name; ?>&#039;s profile"><i class="fa fa-eye" aria-hidden="true"></i> View</a></td>
<?php if(check_admin("change_clients_picture") == 1){ ?>
...
?>
<div class="back"><a href="<?php echo $admin; ?>manage-clients?pn=<?php echo $pn ?>" class="btn gen-btn general-link"><i class="fa fa-arrow-left" aria-hidden="true"></i> Back to clients list</a></div>
<?php
if($count > 0){
...
$result = $db->select("reg_users", "WHERE id='$view' AND client = '1'", "*", "");
...
<div class="back"><a href="<?php echo $admin; ?>manage-clients?pn=<?php echo $pn ?>" class="btn gen-btn general-link float-left"><i class="fa fa-arrow-left" aria-hidden="true"></i> Back to clients list</a></div>
<table class="table table-striped table-hover">
...
if(check_admin("edit_clients") == 1){

if($active == 0){ ?> <a href="<?php echo $admin; ?>manage-clients?view=<?php echo $id; ?>&activate=<?php echo $get_id; ?>&pn=<?php echo $pn ?>" class="btn gen-btn general-link"><i class="fa fa-check" aria-hidden="true"></i> Activate user</a> <?php }

if($blocked == 0){ ?><a href="<?php echo $admin; ?>manage-clients?view=<?php echo $get_id; ?>&block=<?php echo $get_id; ?>&pn=<?php echo $pn ?>" class="btn gen-btn general-link"><i class="fa fa-times" aria-hidden="true"></i> Block user</a> <?php } 

if($blocked == 1){ ?> <a href="<?php echo $admin; ?>manage-clients?view=<?php echo $get_id; ?>&unblock=<?php echo $get_id; ?>&pn=<?php echo $pn ?>" class="btn gen-btn general-link"><i class="fa fa-check" aria-hidden="true"></i> Unblock user</a> <?php } ?> 
...
<a href="<?php echo $admin; ?>manage-clients?edit=<?php echo $get_id; ?>&pn=<?php echo $pn ?>" class="btn gen-btn general-link"><i class="fa fa-pencil" aria-hidden="true"></i> Edit Profile</a>
<?php } ?>
</td></tr>
<tr><td colspan="2">

<?php if(check_admin("edit_clients") == 1){ ?>

<form action="<?php echo $admin; ?>manage-clients" class="general-form reset-row" id="form-div" method="post" runat="server" autocomplete="off" enctype="multipart/form-data" style="width:100%;">  
...
//=======================Add New User==============================//
if(check_admin("add_clients") == 1 && !empty($add) && $error == 1){ ?>
...
<div><a href="<?php echo $admin; ?>manage-clients?pn=<?php echo $pn; ?>" class="btn gen-btn general-link float-left"><i class="fa fa-arrow-left" aria-hidden="true"></i> Back to clients list</a></div>

<form action="<?php echo $admin; ?>manage-clients" method="post" class="general-form" id="form-div" runat="server" autocomplete="off" enctype="multipart/form-data">
<div class="body-header">Add a <span>New Client</span></div>    
...
<div class="form-group input-group">
<span class="input-group-addon"><i class="fa fa-phone" aria-hidden="true"></i></span>
<input type="text" name="mobile" id="mobile" class="form-control" placeholder="Mobile no" value="<?php check_inputted("mobile"); ?>">
</div>
</div>
...
<div class="form-group input-group">
<span class="input-group-addon"><i class="fa fa-user" aria-hidden="true"></i></span>
<input type="text" name="contact_person" id="contact_person" class="form-control" placeholder="Contact person" value="<?php check_inputted("contact_person"); ?>">
</div>
</div>
...
<div class="form-group input-group">
<span class="input-group-addon"><i class="fa fa-user" aria-hidden="true"></i></span>
<input type="text" name="staff_id" id="staff_id" class="form-control" placeholder="Staff ID" value="<?php check_inputted("staff_id"); ?>">
</div>
</div>
...
//=======================Edit User==============================//
if(check_admin("edit_clients") == 1 && !empty($edit) && $error == 1){ 
...
<div><a href="<?php echo $admin; ?>manage-clients?view=<?php echo $edit; ?>&pn=<?php echo $pn; ?>" class="btn gen-btn general-link float-left"><i class="fa fa-arrow-left" aria-hidden="true"></i> Back to client&#039;s profile</a></div>

<form action="<?php echo $admin; ?>manage-clients" method="post" class="general-form" id="form-div" runat="server" autocomplete="off" enctype="multipart/form-data">
<div class="body-header">Edit <span>Client&#039;s Profile</span></div>    
