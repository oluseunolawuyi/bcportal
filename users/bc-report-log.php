...
$result = $db->select("bc_reports_log", "$where", "*", "ORDER BY id DESC");

$per_view = 20;
$per_view = (!empty($no_of_rows))?$no_of_rows:$per_view;
$page_link = "{$admin}bc-report-log?pn=";
$link_suffix = "";
$style_class = "general-link";
page_numbers();
?>

<div class="page-title">Background Checks Report Log</div>

<form action="<?php echo $admin; ?>bc-report-log" name="search_form" class="general-form" id="form-div" method="post" runat="server" autocomplete="off" enctype="multipart/form-data">
...
<?php 
$result2 = $db->select("bc_reports_log", "", "DISTINCT client", "ORDER BY client ASC");
...
<?php 
$result2 = $db->select("bc_reports_log", "", "DISTINCT verification_type", "ORDER BY verification_type ASC");
if(count_rows($result2) > 0){
while($row2 = fetch_data($result2)){
...
$result = $db->select("bc_reports_log", "$where", "*", "ORDER BY id DESC", "LIMIT {$offset},{$per_view}");
